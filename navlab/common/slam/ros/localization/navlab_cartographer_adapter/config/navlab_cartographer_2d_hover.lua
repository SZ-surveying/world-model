...
-- Hover-only profile.
--
-- Hover localization must come from live scan/IMU evidence. The odometry input
-- used here is a scan-reference odometry prior derived only from /scan, not a
-- Gazebo truth pose, fixed XY prior, or official maze-map localizer.
...
-- Keep pose graph optimization on so the scan-derived odometry input is not
-- only an extrapolator hint. The odometry translation weight is intentionally
-- stronger than the local SLAM pose prior because the current hover failure is
-- local SLAM collapsing visible scan drift back toward zero.
