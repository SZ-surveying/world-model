...
class FakeExternalNavNode : public rclcpp::Node {
 public:
  FakeExternalNavNode() : Node("fake_external_nav_node") {
    ...
    status_topic_ = declare_parameter<std::string>("status_topic", "/fake_external_nav/status");
    ...
    timer_ = create_wall_timer(
        ...
        std::bind(&FakeExternalNavNode::publish_tick, this));

    RCLCPP_INFO(get_logger(),
                "fake_external_nav started; mode=%s rate=%.3f odom_topic=%s frame=%s child=%s",
...
int main(int argc, char **argv) {
  ...
  rclcpp::spin(std::make_shared<FakeExternalNavNode>());
  rclcpp::shutdown();
  return 0;
}
