...
class ImuBridgeNode : public rclcpp::Node {
 public:
  ImuBridgeNode() : Node("imu_bridge_node") {
    ...
    status_pub_ = create_publisher<std_msgs::msg::String>("/imu/status", 10);

    if (source_mode_ == "topic") {
      imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
          source_topic_, rclcpp::SensorDataQoS(),
          std::bind(&ImuBridgeNode::handle_input_imu, this, std::placeholders::_1));
    ...
    timer_ = create_wall_timer(
        ...
        std::bind(&ImuBridgeNode::tick, this));

    RCLCPP_INFO(get_logger(),
                "imu_bridge started; mode=%s source_label=%s source_topic=%s",
