...
from navlab.companion.acceptance import (
    ...
    _collect_samples,
    _load_gazebo_truth_trajectory_summary,
    _pose_mirror_set_pose_disabled,
    _record_gazebo_truth_trajectory,
    _record_profile_topics,
    _sample_payload,
    _slam_truth_comparison,
    _wait_for_metadata,
    _write_effective_rosbag_profile,
    _write_foxglove_notes,
    _x2_scan_ideal_fresh,
)
...
def _run_hover_mission(
    *,
    ...
    from navlab.companion.nodes.hover_mission import run as run_hover_mission
...
def _relay_gazebo_truth_to_external_nav(*, manager: ProcessManager, artifact_dir: Path) -> None:
    ...
    manager.start_subprocess(
        ...
        [
            "python3",
            "-m",
            "navlab.companion.nodes.odom_relay",
            "--input-topic",
            "/gazebo/truth/odom",
            "--output-topic",
            "/external_nav/odom",
...
def _write_hover_summary(
    *,
    ...
    slam_truth_error = slam_truth_comparison.get("horizontal_error_m")
    slam_truth_error_ok = (
        True
        if max_slam_truth_horizontal_error_m is None
        else (
            slam_truth_comparison.get("available") is True
    ...
    pose_mirror_observation_only = (
        _pose_mirror_set_pose_disabled(config)
...
def execute_hover_diagnostic_acceptance(
    *,
    ...
    topics = _build_rosbag_topic_plan(
        rosbag_profile_path,
