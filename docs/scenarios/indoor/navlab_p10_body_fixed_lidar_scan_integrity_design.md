...
因此 P10 不能和“探索更远、策略更聪明”混在一起做。先把 scan 输入契约做硬，再处理中等倾角下的 2D scan 稳定化；更激进的探索策略优化顺延到 P12+。
...
- P12：电机/桨叶/ESC 偏置鲁棒性仿真，加入 motor bias / thrust bias / vibration profile，并恢复更激进的 exploration strategy 优化。
- P13：真机 tethered indoor preflight，先做限位/保护下的 hover + scan integrity + stabilization 验收。
