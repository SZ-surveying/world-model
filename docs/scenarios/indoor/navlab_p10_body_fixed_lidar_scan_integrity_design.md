...
因此 P10 不能和“探索更远、策略更聪明”混在一起做。先把 scan 输入契约做硬，再继续做更激进的探索。原先计划中的“真 lidar 探索策略优化”顺延到 P11。
...
- 不重写探索策略，不追求更大覆盖率。
- 不解决所有电机动力学建模问题；电机/桨叶/ESC 偏置鲁棒性可以在后续 P12 单独做。
...
这个补强的完成口径是“可观测性清楚”，不是“电机偏置已经被补偿”。真正的 motor bias / ESC lag / thrust multiplier 鲁棒性仿真仍然放到后续阶段。
...
- P11：恢复原计划的真 lidar exploration strategy 优化，让探索走得更远、更稳。
- P12：电机/桨叶/ESC 偏置鲁棒性仿真，加入 motor bias / thrust bias / vibration profile。
- P13：真机 tethered indoor preflight，先做限位/保护下的 hover + scan integrity 验收。
