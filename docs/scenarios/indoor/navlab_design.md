...
- 由 companion computer 处理 ExternalNav 桥接、传感器特征、避障状态机、MAVLink mission 和 rosbag
- 由独立 SLAM 镜像处理 `/scan` + FCU IMU 到 `/odom` 的定位后端
- 由 Gazebo 提供无 GPS 室内世界、障碍物、可视化 UAV 和理想 scan
- 由 Gazebo/sensor runtime 把理想 scan 转成 X2 虚拟串口，再由厂商 driver 发布最终 `/scan`
...
- 运行或托管 `imu_bridge`、`external_nav_bridge`、scan feature publisher 和 mission controller
...
- 安装或运行具体 SLAM 后端
- 安装 YDLidar SDK 或 X2 虚拟串口仿真器
- 直接读取 Gazebo RaySensor 内部数据
...
- 作为可替换定位算法容器运行
- 消费最终 `/scan`、FCU IMU、`/tf` 等输入
- 发布 `/odom`、`/tf`、`/cartographer/status` 或等价健康状态
- 只通过 ROS graph 和 companion/Gazebo sensor 交互

当前默认镜像是 `world-model/navlab-slam-cartographer:latest`。后续可替换为 FAST-LIO、
VINS、RTAB-Map 或自研定位后端，只要保留 `/scan`、IMU、`/odom`、状态 topic 的接口契约。
...
### 3.5 Gazebo/sensor runtime

职责：

- 启动 `/scan_ideal` 的 ROS-Gazebo bridge
- 启动 X2 虚拟串口 emulator
- 启动厂商 `ydlidar_ros2_driver_node`
- 发布最终 `/scan` 和 `/sim/x2/status`

不负责：

- 下发飞控 setpoint
- 运行 mission controller
- 运行 SLAM 后端
...
Decision: 删除独立通用仿真 runtime 编排和 `sim` CLI；`world_marker_publisher` 与 `scan_features_publisher` 作为 NavLab companion runtime 的 function process 启动。NavLab acceptance 不再启动单独 runner 容器，而是 `docker exec` 到同一个 `navlab-companion` 容器执行录包、mission 和 summary 生成。
...
Decision: `orchestration/config.toml` 新增 NavLab images 配置，统一定义 companion/SLAM/gazebo-sensor 的 repository、tag strategy、Dockerfile、build context 和 build target；`justfile` 只调用 `python orchestration/main.py build ...`，不再直接写 `docker build` 参数。

Basis: `orchestration/config.toml` 里的 `companion_image` / `[slam].image` 是运行时覆盖项；如果 justfile 的 build tag 写死，就会出现 build 和 run 指向不同镜像的维护风险。
