...
- [x] invalid config 不 fallback 到 clean。
...
- [x] 支持 clean profile：`[1.0, 1.0, 1.0, 1.0]`。
- [x] 支持 mild_bias profile。
- [x] 支持 nominal_realistic profile。
...
- [x] clean profile 不改变当前 P11 baseline 行为。
- [x] mild_bias profile 能造成可观测 roll/pitch 差异。
- [x] nominal_realistic profile 能在 P9 representative replay 中运行。
...
- [x] vibration profile 不能用 clean IMU run 冒充。
...
- [ ] P12 summary 记录 P10 scan integrity 指标。
...
## P12.7 Clean baseline

- [x] 运行 P9 representative replay + clean disturbance profile。
- [x] 记录 clean scan/SLAM/ExternalNav/FCU health。
- [x] 记录 clean map artifact score。
- [x] clean baseline summary 写入 P12 summary。
- [x] clean baseline fail 时停止 required disturbed success 判定。

验收：

- [x] clean baseline `ok=true`。
- [x] clean baseline `uses_gazebo_truth_as_input=false`。
- [x] clean baseline `owner.unique=true`。
- [x] clean baseline `set_pose_count=0`。

## P12.8 Disturbed profile acceptance

- [x] 运行 `mild_bias` profile。
- [x] 运行 `nominal_realistic` profile。
- [x] 运行 `esc_lag` profile。
- [x] 运行 `vibration` profile。
- [x] 每个 profile 记录 flight attitude metrics。
- [x] 每个 profile 记录 scan drop/compensate/floor-hit 指标。
...
- [x] profile-sweep summary 记录 disturbed-vs-clean aggregate comparison。

验收：

- [x] required profiles 全部运行。
- [x] required profiles 没有静默 fallback 到 clean。
...
- [x] invalid_config 不允许 fallback 到 clean。
...
- [x] summary 记录 disturbed-vs-clean comparison。
...
- [x] 逐个 profile 跑 full live P9 replay：`mild_bias`。
- [x] 逐个 profile 跑 full live P9 replay：`esc_lag`。
- [x] 逐个 profile 跑 full live P9 replay：`vibration`。
...
- [x] `just navlab-airframe-disturbance-gate-acceptance 240 --live-profiles mild_bias,esc_lag,vibration` 通过。
- [x] 每个 live profile 的 P11 nested summary `ok=true`。
- [x] 每个 live profile 的 rosbag profile `ok=true`。
...
- [x] clean baseline ok。
- [x] mild_bias / nominal_realistic / esc_lag / vibration required profiles 全部 evaluated。
...
- scan 结果：`passthrough_scan_count=605`，`compensated_scan_count=0`，`dropped_scan_count=0`，`max_observed_tilt_deg=2.877`，`false_wall_risk_ok=true`。补偿未触发原因是 `tilt_never_exceeded_passthrough_tilt_deg`，说明该 nominal-realistic live run 仍被 FCU 压在 passthrough 区。
...
- 边界：live 只跑了当前 `nominal_realistic` 配置 profile；`mild_bias/esc_lag/vibration` 已在 profile-sweep gate evaluated，但还没有逐个做 full live P9 replay。
...
- 验证：`PYTHONPATH=orchestration uv run --project orchestration pytest orchestration/tests/test_airframe_disturbance_gate.py orchestration/tests/test_config.py -q` 通过；`just navlab-airframe-disturbance-gate-doctor` 通过；`just navlab-airframe-disturbance-gate-acceptance 240 --live-profiles mild_bias,esc_lag,vibration` 通过。
- artifact：`artifacts/ros/navlab_companion_sitl_gazebo/20260608_191029/summary.json`，P12 `ok=true`、`blockers=[]`。
- FCU mode gate：`mild_bias` 72/72 GUIDED、`esc_lag` 71/71 GUIDED、`vibration` 70/70 GUIDED；三个 profile 的 `non_guided_count=0`、`schema_invalid_count=0`。

### 2026-06-08 P12 first-order ESC lag + clean/nominal live closure

- 范围：复核 P12 ESC lag 实现与设计一致性，并补跑 `clean` / `nominal_realistic` live replay。
- ESC lag 证据：当前 SDF overlay 写入 `<escTimeConstantMs>`，`model_overlay.airframe_disturbance.applied_controls[*].esc_lag_model=plugin_first_order`，`esc_lag_claim=first_order`；`artifacts/ros/navlab_companion_sitl_gazebo/20260608_195430/p12_live_nominal_realistic_replay/iris_with_lidar_p11_rangefinder_x2.sdf` 中 `escTimeConstantMs` 出现 8 次，未出现 `frequencyCutoff` proxy。
- clean artifact：`artifacts/ros/navlab_companion_sitl_gazebo/20260608_200320/summary.json`，P12 `ok=true`、`blockers=[]`；`clean` live `passthrough=606`、`compensated=0`、`dropped=0`、`max_observed_tilt_deg=2.991`、FCU 71/71 GUIDED。
- nominal artifact：`artifacts/ros/navlab_companion_sitl_gazebo/20260608_195430/summary.json`，P12 `ok=true`、`blockers=[]`；`nominal_realistic` live `passthrough=550`、`compensated=40`、`dropped=20`、`max_observed_tilt_deg=5.793`、`false_wall_risk_ok=true`、FCU 73/73 GUIDED。
- Foxglove-lite：已为 `clean`、`nominal_realistic`、`mild_bias`、`esc_lag`、`vibration` 的最新 live replay 生成 P12 lite MCAP，`foxglove_replay_summary.json` 均为 `ok=true`。
