...
class HoverDiagnosticTask(OrchestrationTask):
    TASK_NAME: ClassVar[str] = "hover-diagnostic"
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            console.print(
                f"[bold cyan]Running rangefinder hover diagnostic inside companion for {duration_sec:g}s[/bold cyan]"
            )
            rc = host._docker_exec_runtime_command(
                module="navlab.companion.acceptance_cli",
                args=[
                    "execute-hover-diagnostic-acceptance",
                    "--artifact-dir",
                    str(config.artifact_dir),
                    "--duration-sec",
                    str(duration_sec),
                    "--rosbag-profile",
                    "profiles/navlab-hover-diagnostic-rosbag-topics.txt",
                    "--companion-image",
            ...
            finalize_navlab_artifact(
                ...
                rosbag_profile="profiles/navlab-hover-diagnostic-rosbag-topics.txt",
                session_log_dir=host._session_log_dir(config),
                stage_label="FCU rangefinder hover diagnostic",
                control_mode="guided_rangefinder_altitude_no_slam_horizontal_control",
            )
        ...
        console.print(f"[{color}]NavLab hover diagnostic completed rc={rc}[/{color}]")
