...
class HoverSlamDiagnosticTask(OrchestrationTask):
    TASK_NAME: ClassVar[str] = "hover-slam-diagnostic"
    TASK_DESCRIPTION: ClassVar[str] = "Run NavLab SLAM ExternalNav position-hold hover diagnostic."
    ...
    def run(
        ...
        *,
        ...
        rc = 1
        try:
            console.print("[bold cyan]Starting SITL + Gazebo + SLAM + companion stack[/bold cyan]")
            host._compose_up(config)
            host._start_slam_container(config)
            host._start_companion_container(config)
            console.print("[bold cyan]Running SLAM ExternalNav hover diagnostic inside companion[/bold cyan]")
            rc = host._docker_exec_runtime_command(
                module="navlab.companion.acceptance_cli",
                args=[
                    "execute-hover-slam-diagnostic-acceptance",
                    "--artifact-dir",
            ...
            finalize_navlab_artifact(
                ...
                stage_label="SLAM ExternalNav hover diagnostic",
                control_mode="guided_rangefinder_altitude_slam_external_nav_position_hold",
            )
        ...
        console.print(f"[{color}]NavLab SLAM hover diagnostic completed rc={rc}[/{color}]")
