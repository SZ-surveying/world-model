...
from lab_env.sim.nodes import waypoint_follower
from lab_env.sim.nodes.waypoint_follower import compute_auto_run_decision
...
def test_stop_auto_rosbag_if_configured_sends_sigint_and_waits(monkeypatch) -> None:
    ...
    def fake_killpg(pid: int, sig: int) -> None:
        if sig == waypoint_follower.signal.SIGINT:
    ...
    monkeypatch.setattr(waypoint_follower.os, "killpg", fake_killpg)
    monkeypatch.setattr(waypoint_follower.time, "sleep", lambda _seconds: None)

    waypoint_follower._stop_auto_rosbag_if_configured(timeout_sec=0.1)

    assert signals == [(4242, waypoint_follower.signal.SIGINT)]
